...
public class GraficadorController extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		HashMap<String, StackPane> stacks = new HashMap<String, StackPane>();

		for (Usuario usuario : singleton.getRedSocial().getGrafoVendedores().getAllVertices()) {

			String aux = usuario.printInitials();

			StackPane stack = getDot("pink", aux);
			stacks.put(aux, stack);
		}

		for (Entry<String, StackPane> entry : stacks.entrySet()) {
			String key = entry.getKey();
			StackPane value = entry.getValue();

			// pane.getChildren().addAll(value);
			...
			for (Usuario usuario : singleton.getRedSocial().getGrafoVendedores().getAllVertices()) {
				...
				if (key.equals(aux)) {
					...
					for (Usuario usuario2 : singleton.getRedSocial().getGrafoVendedores().getNeighbors(usuario)) {

						StackPane destino = retornoStack(stacks, usuario2.printInitials());

						buildSingleDirectionalLine(destino, value, pane, false, true); // C --> D
		...
		/*
		 * for (Usuario usuario :
		 * singleton.getRedSocial().getGrafoVendedores().getAllVertices()) { String aux
		 * = usuario.printInitials();
		 * 
		 * 
		 * if (key.equals(aux)) {
		 * 
		 * for (Usuario usuario2 :
		 * singleton.getRedSocial().getGrafoVendedores().getNeighbors(usuario)) {
		 * 
		 * StackPane destino = retornoStack(stacks, usuario2.printInitials());
		 * 
		 * buildSingleDirectionalLine(value, destino, pane, true, false); // C --> D
		 * **** }
		 * 
		 * }
		 * 
		...
		/*
		 * StackPane dotA = getDot("green", "A"); StackPane dotB = getDot("red", "B");
		 * StackPane dotC = getDot("yellow", "C"); StackPane dotD = getDot("pink", "D");
		 ...
		 * 
		 * // D <===> E buildBiDirectionalLine(true, dotD, dotE, pane);
	...
	public StackPane retornoStack(HashMap<String, StackPane> map, String iniciales) {

		StackPane stack = new StackPane();

		for (Entry<String, StackPane> entry : map.entrySet()) {
			String key = entry.getKey();
			StackPane value = entry.getValue();

			if (key.equals(iniciales)) {
